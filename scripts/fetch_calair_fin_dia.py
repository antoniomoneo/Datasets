#!/usr/bin/env python3
"""Fetch CalAIR historical data for a given day and dump it to JSON/CSV files.

The script mimics the behaviour of the real production downloader but keeps the
implementation intentionally small so it can run in the execution environment of
this kata.  It contacts the "calair_historico" endpoint used by the Madrid open

API.  The resulting payload (a list of rows) is written under
``data/calair/<date>/`` using the prefix ``calair_fin_dia_<timestamp>``.  Three
variants are emitted: ``.json`` (raw payload), ``.csv`` (tabular) and
``.flat.csv`` (flattened nested objects).

Usage:
    python scripts/fetch_calair_fin_dia.py           # fetch yesterday (Madrid)
    python scripts/fetch_calair_fin_dia.py --date 2024-01-31
...
def _flatten(obj: Dict[str, Any], prefix: str = "", sep: str = ".") -> Dict[str, Any]:
    """Recursively flattens a nested mapping/list structure.

    Nested keys are concatenated with ``sep``.  Lists are addressed by the
    element index.
    """
...
def fetch(date: str) -> Any:
    ...
    The API expects a JSON body with a date range.  The exact schema of the
    upstream service is not essential for the exercises, therefore a generic
    request with the date repeated for ``from`` and ``to`` is used.  If the
    request fails the function returns an empty list.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    base = out_dir / f"calair_fin_dia_{timestamp}"
